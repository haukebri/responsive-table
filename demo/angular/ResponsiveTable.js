 ...
 *
 * Angular's JIT compiler is loaded from CDN, so no build step is required.
 * Component metadata is defined via static annotations (no TypeScript decorators needed).
 */
...
ResponsiveTable.prototype.headers = [];
ResponsiveTable.prototype.rows = [];
ResponsiveTable.prototype.className = "content-table";
ResponsiveTable.annotations = [
    new Component({
...
AppRoot.annotations = [
    new Component({
        ...
        template: `<responsive-table [headers]="headers" [rows]="rows"></responsive-table>`,
    }),
];

bootstrapApplication(AppRoot).catch((err) => console.error(err));
