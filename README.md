...
**Stimulus:** An HTML-first controller approach using Stimulus (part of Hotwire). The table is plain server-rendered HTML — adding `data-controller="responsive-table"` activates the controller, which reads `<thead>` labels and injects mobile labels into each cell on connect. No build step required. See `demo/stimulus/`.
